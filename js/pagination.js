...
$(document).ready(function(){
	...
	var number_of_items = $('#video-place').children().size();
	...
	*/
	...
	$('#video-place').children().css('display', 'none');
	
	//and show the first n (show_per_page) elements
	$('#video-place').children().slice(0, show_per_page).css('display', 'block');
...
var show_per_page = $('#show_per_page').val();
	//calculate the number of pages we are going to have
	var number_of_pages = Math.ceil(number_of_items/show_per_page);
	var navigation_html = '<a class="previous_link" href="javascript:previous();">Previous Video</a>';
	...
	navigation_html += '<a class="next_link" href="javascript:next();">Next Video</a>';
...
function go_to_page(page_num){
	...
	$('#video-place').children().css('display', 'none').slice(start_from, end_on).css('display', 'block');
